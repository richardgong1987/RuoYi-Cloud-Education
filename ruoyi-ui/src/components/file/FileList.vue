...
import {getFileListByPath, getFileListByType, getMyShareFileList, getRecoveryFile, searchFile} from '_r/file'
...
export default {
  ...
  methods: {
     ...
     */
    showFileRecovery() {
      getRecoveryFile({userId: this.$route.params && this.$route.params.userId}).then((res) => {
        if (res.success) {
